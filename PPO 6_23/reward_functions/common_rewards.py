...
class GoalReward(RewardFunction):
    ...
    def get_reward(self, player: PlayerData, state: GameState, previous_action: np.ndarray, optional_data=None):
        ...
        if player.team_num == BLUE_TEAM:
            ...
            #              (self.per_goal * d_player + self.team_score_coeff * d_blue - self.concede_coeff * d_orange) *
            #               float((np.linalg.norm(player.car_data.position - objective)) / 100)]
            # print("GoalRewardBlue:  ", ((self.per_goal * d_player + self.team_score_coeff * d_blue - self.concede_coeff * d_orange) *
            #               float((np.linalg.norm(player.car_data.position - objective)) / 100)))
            if ((self.per_goal * d_player) * float(np.linalg.norm(player.car_data.position - objective))) > 0:
                print("Goal_Reward_Blue:  ", ((self.per_goal * d_player) * float(np.linalg.norm(player.car_data.position - objective))))
            return ((self.per_goal * d_player) * float(np.linalg.norm(player.car_data.position - objective)))
        else:
            objective = np.array(BLUE_GOAL_CENTER)
            objective[1] *= 5560 / 5120
            # get_rewards = [player.car_id, "goal_reward",
            #               (self.per_goal * d_player + self.team_score_coeff * d_orange - self.concede_coeff * d_blue) *
            #                float((np.linalg.norm(player.car_data.position - objective)) / 100)]
            # print("GoalRewardOrange:  ", ((self.per_goal * d_player + self.team_score_coeff * d_orange - self.concede_coeff * d_blue) *
            #                float((np.linalg.norm(player.car_data.position - objective)) / 100)))
            if ((self.per_goal * d_player) * float(np.linalg.norm(player.car_data.position - objective))) > 0:
                print("Goal_Reward_Orange:  ", ((self.per_goal * d_player) * float(np.linalg.norm(player.car_data.position - objective))))
            return ((self.per_goal * d_player) * float(np.linalg.norm(player.car_data.position - objective)))
    ...
    def get_final_reward(self, player: PlayerData, state: GameState, previous_action: np.ndarray, optional_data=None):
        ...
        if player.team_num == BLUE_TEAM:
            ...
            #               float((np.linalg.norm(player.car_data.position - objective)) / 100)))
            if ((self.per_goal * d_player) * float(np.linalg.norm(player.car_data.position - objective))) > 0:
                print("Goal_Reward_Blue_FINAL:  ", ((self.per_goal * d_player) * float(np.linalg.norm(player.car_data.position - objective))))
            return ((self.per_goal * d_player) * float(np.linalg.norm(player.car_data.position - objective)))
        else:
            objective = np.array(BLUE_GOAL_CENTER)
            objective[1] *= 5560 / 5120
            ...
            #                float((np.linalg.norm(player.car_data.position - objective)) / 100)))
            if ((self.per_goal * d_player) * float(np.linalg.norm(player.car_data.position - objective))) > 0:
                print("Goal_Reward_Orange_FINAL:  ", ((self.per_goal * d_player) * float(np.linalg.norm(player.car_data.position - objective))))
            return ((self.per_goal * d_player) * float(np.linalg.norm(player.car_data.position - objective)))
